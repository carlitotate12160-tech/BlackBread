...
def _parse_code_scanning(raw_scans: Any) -> tuple[CodeScanningRequirement, ...]:
    ...
    parsed = []
    seen = set()
    valid_security_thresholds = {"none", "all", "medium_or_higher", "high_or_higher", "critical"}
    valid_alerts_thresholds = {"none", "all", "errors_and_warnings", "errors"}
    for rs in raw_scans:
        _check_type(rs, dict)
        ...
        if rs["security_alerts_threshold"] not in valid_security_thresholds:
            _fail_exit_2("CONTRACT_INVALID_CODE_SCANNING")
        if rs["alerts_threshold"] not in valid_alerts_thresholds:
            _fail_exit_2("CONTRACT_INVALID_CODE_SCANNING")
        if rs["tool"] in seen:
            _fail_exit_2("CONTRACT_INVALID_CODE_SCANNING")
        seen.add(rs["tool"])
