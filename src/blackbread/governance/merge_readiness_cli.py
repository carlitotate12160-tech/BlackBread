...
from blackbread.governance.github_merge_transport import UrllibGitHubReadTransport, TransportError
from blackbread.governance.github_merge_evidence import GitHubMergeEvidenceCollector, EvidenceCollectionError
...
def _parse_status_checks(raw_checks: Any) -> tuple[RequiredStatusCheck, ...]:
    ...
    for rc in raw_checks:
        ...
        parsed.append(RequiredStatusCheck(
            context=rc["context"],
            integration_id=rc["integration_id"]
        ))
    return tuple(parsed)
...
def _parse_code_scanning(raw_scans: Any) -> tuple[CodeScanningRequirement, ...]:
    ...
    for rs in raw_scans:
        ...
        parsed.append(CodeScanningRequirement(
            tool=rs["tool"],
...
def _load_contract() -> DeliveryContract:
    try:
        with open(".github/agent-delivery.json", "r", encoding="utf-8") as f:
    ...
    for k in _EXPECTED_AD_KEYS:
        if k not in {"required_status_checks", "required_code_scanning", "required_approving_reviews", "ruleset_id"}:
...
def main(argv: list[str] | None = None) -> None:
    ...
    # The token is never logged or emitted.
    # The error boundary catches all exceptions.
    try:
        ...
        if all_allowlisted and "INCOMPLETE_EVIDENCE" not in blocker_codes:
            # wait, if "INCOMPLETE_EVIDENCE" is in blockers, then all_allowlisted would be false anyway
            # but to be sure, any partial evidence returns exit 2.
            pass
        ...
        sys.stdout.write(json.dumps(out_1, sort_keys=True, separators=(',', ':')) + "\n")
        sys.exit(1)
        ...
        # Sanitize all unexpected exceptions to exit 2 without details.
