...
async def _no_snapshot_hook(connection: AsyncConnection) -> None:
    del connection


# A deliberately narrow scheduling seam. Tests may replace it, but all ledger reads
# and PostgreSQL behavior remain real.
_after_snapshot_anchor: Callable[[AsyncConnection], Awaitable[None]] = _no_snapshot_hook
...
async def _verify_in_snapshot(
    ...
    *,
    ...
    # The anchor SELECT above is intentionally the first data statement and forms
    # the snapshot. Do not move this seam before that read.
    await _after_snapshot_anchor(connection)
    ...
    try:
        async for row in stream.mappings():
            ...
            event_count += 1
            failure = _first_failure(event, expected_sequence, tenant_id, expected_prev)
