...
# Runtime-gate reasons. Proposal-time, tenant/engagement, capability, identity, scope, and
# structural-budget failures are owned by the internally computed admission and surface as
# ADMISSION_DENIED, so they are not duplicated here. Every reason below concerns a runtime fact
# (RuntimeGateSnapshot) that admission does not evaluate, plus the runtime-snapshot binding.
...
# Stale-context reasons: freshness, coherence, and missing/expired runtime-state facts.
...
def _outcome_for_reason(reason: RuntimeGateReason) -> RuntimeGateOutcome:
    ...
    if reason.startswith("OPSEC_") and reason != "OPSEC_STATE_EXPIRED":
        return "OPSEC_HOLD"
    return "STALE_CONTEXT" if reason in _STALE_REASONS else "DENY"
