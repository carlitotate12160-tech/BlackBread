...
- **Independent AI review is PR-Agent (CodiumAI)** via `.github/workflows/pr-agent.yml` (`DEEPSEEK_API_KEY`); automatic runs are not guaranteed by config or SaaS availability. Per ADR-FINAL-002 Amendment A-001, PR-Agent uses a tiered model: `deepseek/deepseek-v4-pro` for PRs labeled `safety-critical` (binding review), `deepseek/deepseek-v4-flash` for all other PRs (advisory). For safety-critical changes the V4-Pro review is binding — see *Repository delivery authority*.
