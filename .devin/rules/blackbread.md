...
- **Comments and docstrings:** Prefer self-explanatory code. Add or preserve concise comments/docstrings when they explain a non-obvious invariant, authority boundary, security rationale, concurrency/TOCTOU rule, provenance requirement, compatibility constraint, protocol preimage, or why a safer-looking alternative is incorrect. Do not add narration that merely restates syntax or implementation steps. Never delete an accurate explanatory comment or docstring merely to satisfy style, line, or diff budgets; split responsibilities when honest explanation no longer fits. Do not use emojis in code or files.
