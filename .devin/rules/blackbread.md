...
- The mandatory first-party CI gates are `quality`, `tests`, `security`, and `governance`. Qodo is the primary automated reviewer and CodeRabbit is the independent reviewer for safety-critical paths; both post advisory findings that must be dispositioned before merge, but no AI reviewer is a required status check and none replaces a first-party CI gate.
