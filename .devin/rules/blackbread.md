...
- Run `ruff check`, `ruff format --check`, `mypy --strict`, `pytest --cov=blackbread --cov-fail-under=80`, `bandit -r src/blackbread`, and `pip-audit` before considering work done. Required jobs are `quality`, `tests`, `security`, and `governance`; branch protection must require them and Code Owner approval. No required job may be skipped, allowed to fail, or replaced by an AI reviewer.
