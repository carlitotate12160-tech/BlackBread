...
- The mandatory first-party CI gates are `quality`, `tests`, `security`, and `governance`. The additional blocking third-party review check is `Sourcery review`; it does not replace any first-party CI gate. Every actionable Sourcery finding requires an evidence-backed disposition before merge, and unresolved actionable findings remain merge blockers.
- CodeRabbit, Codex, Bito, and any other configured AI review or comment mechanisms are distinct from the required `Sourcery review` status context. Their actionable findings still require disposition, but they neither substitute for Sourcery nor replace mandatory first-party CI.
- Implementation agents may not bypass, remove, rename, or ignore `Sourcery review` because of outage, timeout, quota, missing installation, slowness, unavailability, or inconvenience. Any emergency or degraded-mode change requires a separate, explicit governance decision with documented evidence; implementation work must otherwise fail closed.
