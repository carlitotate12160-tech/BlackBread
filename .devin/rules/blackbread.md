...
- **Independent AI review is PR-Agent (CodiumAI)** via `.github/workflows/pr-agent.yml` (`DEEPSEEK_API_KEY`, `deepseek/deepseek-v4-pro`); automatic runs are not guaranteed by config or SaaS availability. For safety-critical changes it is binding — see *Repository delivery authority*.
