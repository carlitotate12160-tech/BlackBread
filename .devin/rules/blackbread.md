...
- The mandatory first-party CI gate is `ci-ok` — a single aggregator job that depends on `quality`, `tests`, `security`, and `governance`. Branch protection requires only `ci-ok` and `GitGuardian Security Checks`. When a new required CI job is added to `ci.yml`, it must also be added to `ci-ok.needs` and to `REQUIRED_CI_COMMANDS` in `test_governance_contract.py`; the governance test enforces that every required job appears in `ci-ok.needs`. CodeRabbit is auto-triggered via `coderabbit-trigger.yml` (PAT-owned comment, not github-actions bot) and Qodo is the fallback advisory reviewer; both post advisory findings that must be dispositioned before merge, but neither is a required status check. For non-safety-critical PRs, if CodeRabbit is rate-limited or unavailable, Qodo provides the fallback advisory review; if both are unavailable, the owner dispositions the absence explicitly in the PR and merge is not blocked. For safety-critical PRs, the CodeRabbit FULL review requirement in the next rule is binding and cannot be waived by Qodo fallback or owner disposition alone — an unavailable CodeRabbit review on a safety-critical PR blocks merge until CodeRabbit recovers or an accepted ADR amendment provides a compensating control.
- Safety-critical paths additionally require a current-head CodeRabbit FULL independent review with all actionable findings resolved or dispositioned with recorded evidence. This requirement is binding and cannot be satisfied by Qodo, Codex, Bito, Sourcery, or any other review mechanism alone; those are additional evidence only.
