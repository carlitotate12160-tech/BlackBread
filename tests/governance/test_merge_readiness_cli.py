...
from blackbread.governance.github_merge_transport import UrllibGitHubReadTransport
from blackbread.governance.github_merge_evidence import GitHubMergeEvidenceCollector

# The test suite for the CLI

def run_cli(env: dict[str, str], args: list[str], cwd: str | None = None) -> subprocess.CompletedProcess[str]:
...
def test_invalid_contracts_fail_with_exit_two_before_io(tmp_path: Path):
    ...
    invalid_json_cases = [
        "not_json",
        '{"schema_version": 3, "agent_delivery": {"owner_instruction_required": true, "owner_instruction_required": false}}',
        '{"schema_version": 3, "unknown_key": 1}',
        '{"schema_version": 3}',
        '{"schema_version": 999, "agent_delivery": {}}',
        '{"schema_version": 3, "agent_delivery": {"required_status_checks": [{"context": "ci-ok", "integration_id": "not_an_int"}]}}',
    ]
...
def test_expected_head_is_passed_unchanged_and_mismatch_returns_one(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    with mock.patch("blackbread.governance.merge_readiness_cli.evaluate_merge_readiness") as mock_eval:
        from blackbread.governance.merge_readiness import MergeBlocker
        mock_eval.return_value = MergeReadinessDecision(ready=False, blockers=(MergeBlocker("HEAD_SHA_MISMATCH", "expected != actual"),))
...
def test_complete_ready_evidence_returns_zero_with_canonical_output(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    contract_file = tmp_path / ".github" / "agent-delivery.json"
    contract_file.parent.mkdir()
    contract_file.write_text(json.dumps({
        "schema_version": 3,
    ...
    with mock.patch("blackbread.governance.merge_readiness_cli.evaluate_merge_readiness") as mock_eval:
        ...
        with mock.patch("blackbread.governance.merge_readiness_cli.GitHubMergeEvidenceCollector.collect"):
...
def test_complete_substantive_blockers_return_one(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    contract_file = tmp_path / ".github" / "agent-delivery.json"
    contract_file.parent.mkdir()
    contract_file.write_text(json.dumps({
        "schema_version": 3,
    ...
    from blackbread.governance.merge_readiness import MergeBlocker
    with mock.patch("blackbread.governance.merge_readiness_cli.evaluate_merge_readiness") as mock_eval:
        mock_eval.return_value = MergeReadinessDecision(ready=False, blockers=(MergeBlocker("CHANGES_REQUESTED", "detail"),))
        with mock.patch("blackbread.governance.merge_readiness_cli.GitHubMergeEvidenceCollector.collect"):
            with mock.patch.dict("os.environ", {"GITHUB_TOKEN": "token"}):
                monkeypatch.chdir(tmp_path)
                with pytest.raises(SystemExit) as exc:
        ...
        mock_eval.return_value = MergeReadinessDecision(ready=False, blockers=(MergeBlocker("UNKNOWN_BLOCKER", "detail"),))
        with mock.patch("blackbread.governance.merge_readiness_cli.GitHubMergeEvidenceCollector.collect"):
            with mock.patch.dict("os.environ", {"GITHUB_TOKEN": "token"}):
                monkeypatch.chdir(tmp_path)
                with pytest.raises(SystemExit) as exc:
...
def test_incomplete_or_drifted_evidence_returns_two(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    contract_file = tmp_path / ".github" / "agent-delivery.json"
    contract_file.parent.mkdir()
    contract_file.write_text(json.dumps({
        "schema_version": 3,
    ...
    from blackbread.governance.merge_readiness import MergeBlocker
    with mock.patch("blackbread.governance.merge_readiness_cli.evaluate_merge_readiness") as mock_eval:
        mock_eval.return_value = MergeReadinessDecision(ready=False, blockers=(MergeBlocker("INCOMPLETE_EVIDENCE", "pagination"),))
        with mock.patch("blackbread.governance.merge_readiness_cli.GitHubMergeEvidenceCollector.collect"):
            with mock.patch.dict("os.environ", {"GITHUB_TOKEN": "token"}):
                monkeypatch.chdir(tmp_path)
                with pytest.raises(SystemExit) as exc:
...
def test_token_remote_text_and_exception_text_never_reach_output(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    contract_file = tmp_path / ".github" / "agent-delivery.json"
    contract_file.parent.mkdir()
    contract_file.write_text(json.dumps({
        "schema_version": 3,
...
def test_cli_constructs_urllib_transport_collector_and_existing_evaluator(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    contract_file = tmp_path / ".github" / "agent-delivery.json"
    contract_file.parent.mkdir()
    contract_file.write_text(json.dumps({
        "schema_version": 3,
    ...
    import blackbread.governance.merge_readiness_cli as cli
    with mock.patch("blackbread.governance.merge_readiness_cli.UrllibGitHubReadTransport") as mock_transport:
        with mock.patch("blackbread.governance.merge_readiness_cli.GitHubMergeEvidenceCollector") as mock_collector_cls:
...
def test_smoke_document_names_exact_command_exit_contract_and_redaction():
