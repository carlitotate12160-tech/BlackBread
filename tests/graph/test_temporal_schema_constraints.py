...
class TestConstraints:
    async def test_verified_event_count_minimum(self, admin_engine: AsyncEngine) -> None:
        ...
        async with admin_engine.begin() as conn:
            with pytest.raises(Exception, match="ck_graph_temporal_snapshots_event_count"):
                await conn.execute(
                    text(
    ...
    async def test_lineage_head_sequence_constraints(self, admin_engine: AsyncEngine) -> None:
        ...
        # lineage_head_sequence > verified_event_count → rejected
        async with admin_engine.begin() as conn:
            with pytest.raises(Exception, match="ck_graph_temporal_snapshots_lineage_head_seq_max"):
                await conn.execute(
                    text(
                        "INSERT INTO graph_temporal_projection_snapshots "
                        "(tenant_id, engagement_id, verified_event_count, verified_head_hash, "
    ...
    async def test_revision_predecessor_constraint(self, admin_engine: AsyncEngine) -> None:
        ...
        async with admin_engine.begin() as conn:
            ...
            # v1 with non-NULL predecessor → rejected
            with pytest.raises(Exception, match="ck_graph_temporal_revisions_predecessor"):
                await conn.execute(
                    text(
